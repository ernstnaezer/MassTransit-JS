...
(function(exports){
	...
	function StompTransportFactory(){
		 ...
		 *		Function callback to be triggered when the client disconnects
		 */
		function buildOutbound(address, readyCallback, failureCallback){
			
			var serverAddress = buildServerAddress(address);
						
			// prevent crash
			if(!readyCallback) readyCallback = function(){};
			if(!failureCallback) failureCallback = function(){};
			...
			}, function(){ Log.info("error building connection to: " + serverAddress); failureCallback() });		
		 ...
		 *		Function callback to be triggered when the client disconnects
		 */	
		function buildInbound(address, readyCallback, failureCallback){
